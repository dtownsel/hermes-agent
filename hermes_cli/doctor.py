...
def _build_apikey_providers_list() -> list:
    ...
    # Gemini AI Studio requires `key=`/x-goog-api-key, not Bearer).
...
def run_doctor(args):
    ...
    def _probe_gemini() -> _ConnectivityResult:
        ...
        if not key:
            return _ConnectivityResult("Google Gemini API", [], [])
        label = "Google Gemini API".ljust(20)
        try:
            import httpx
            # Gemini API keys are accepted as a `key=` query parameter (or
            # x-goog-api-key), not as OAuth-style Authorization: Bearer.
            # The generic API-key provider probe intentionally uses Bearer
            # auth for OpenAI-compatible /models endpoints, so Gemini needs a
            # dedicated probe to avoid false "invalid API key" reports.
            r = httpx.get(
            ...
            if r.status_code in (400, 401, 403):
                return _ConnectivityResult(
                    ...
                    [(color("✗", Colors.RED), label,
                      color("(invalid API key)", Colors.DIM))],
                    ["Check GOOGLE_API_KEY or GEMINI_API_KEY in .env"],
                )
            return _ConnectivityResult(
                "Google Gemini API",
    ...
    _probes.append(("Google Gemini API", _probe_gemini))
